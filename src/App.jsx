...
export default function App() {
  ...
  const [phevInsurance, setPhevInsurance] = useState(1500);
  ...
  const c = useMemo(() => {
    ...
    return {
      ...
      opp: { v: 0, p: phevOpp, r: rav4hOpp, h: hybridOpp },
      ...
      volvoInsurance, phevInsurance, rav4hInsurance, hybridInsurance,
      investReturn]);
  ...
  const rows = [
    ...
    { label: "Insurance", v: c.ins.v, p: c.ins.p, r: c.ins.r, h: c.ins.h },
    ...
    { label: `Opportunity cost of capital (${investReturn}%/yr)`, v: c.opp.v, p: c.opp.p, r: c.opp.r, h: c.opp.h, highlight: true },
  ];
  ...
  return (
    <div style={{ fontFamily: "system-ui, sans-serif", maxWidth: 940, margin: "0 auto", padding: 16, fontSize: 13 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "340px 1fr", gap: 16 }}>
        ...
        <div style={{ background: "#f9fafb", borderRadius: 12, padding: 14, border: "1px solid #e5e7eb", fontSize: 12 }}>
          ...
          <Slider label="Apartment rate ($/kWh)" value={homeRate} min={0.20} max={0.55} step={0.005}
            display={v => `$${v.toFixed(3)}`} onChange={setHomeRate}
            note="Confirmed $0.35/kWh · RAV4 Hybrid needs no charging" />
          ...
          <div style={{ background: "#f0fdf4", border: "1px solid #bbf7d0", borderRadius: 8, padding: 9, marginBottom: 10, fontSize: 11, color: "#14532d" }}>
            Opportunity cost = investment growth <em>foregone</em> by spending cash on a car instead. Applies only to net purchase outlay (price − $15k trade-in). Volvo = $0 outlay = $0 opportunity cost.
          </div>
          <Slider label="Expected annual investment return" value={investReturn} min={1} max={15} step={0.5}
            ...
            note={`At ${investReturn}%, ~${fmt(Math.round((42750 - 15000) * (Math.pow(1 + investReturn / 100, years) - 1)))} foregone on a ~$28k outlay over ${years} yr`} />

          <SH t="PHEV Specs" />
          <Slider label="RAV4 PHEV real-world EV range" value={phevEvRange} min={30} max={55} step={1}
            display={v => `${v} mi`} onChange={setPhevEvRange}
            note="Edmunds tested 48 mi (2025). 2026 claims 52 mi." />
          <Slider label="RAV4 PHEV gas-mode MPG" value={phevMPGgas} min={28} max={44} step={1}
            display={v => `${v} mpg`} onChange={setPhevMPGgas}
            note="Mfr 41 mpg. Real-world ~34–38. Default: 38." />

          <SH t="Vehicle Prices (Cash, −$15k Trade-In)" />
          <Slider label="RAV4 PHEV SE (incl. dest.)" value={phevPrice} min={40000} max={52000} step={250}
            display={v => fmt(v)} onChange={setPhevPrice}
            note="SE $42,950 · XSE $48,600 · GR Sport $49,950" />
          <Slider label="⭐ RAV4 Hybrid XSE (AWD, incl. dest.)" value={rav4hPrice} min={38000} max={48000} step={250}
            ...
            note="XSE $42,750 · LE $33,350 · Limited ~$44–45k · Recommended: XSE" />
          <Slider label="Crosstrek Hybrid (Premium trim)" value={hybridPrice} min={36000} max={44000} step={250}
            ...
            note="~$40k premium · Non-plug-in · 36 mpg" />

          <SH t="RAV4 Hybrid MPG" />
          <Slider label="RAV4 Hybrid XSE AWD combined" value={rav4hMPG} min={34} max={47} step={1}
            display={v => `${v} mpg`} onChange={setRav4hMPG}
            note="EPA: 41 mpg AWD. Real-world highway ~38–43." />
          ...
          <Slider label="RAV4 Hybrid XSE" value={rav4hDepRate} min={8} max={22} step={1} display={v => `${v}%`} onChange={setRav4hDepRate} note="RAV4 Hybrid holds value well: ~11–15%/yr" />
          ...
          <div style={{ background: "#fefce8", border: "1px solid #fef08a", borderRadius: 8, padding: 9, marginBottom: 10, fontSize: 11, color: "#713f12" }}>
            Your Volvo quote: $600/6mo = $1,200/yr. New car rates are estimates — plug in your actual quotes once you get them. RAV4s and Subarus tend to insure cheaper than average for their class.
          </div>
          <Slider label="Volvo V90 (confirmed)" value={volvoInsurance} min={600} max={3000} step={50}
            display={v => `${fmt(v)}/yr`} onChange={setVolvoInsurance}
            note="Your confirmed rate: $600/6mo = $1,200/yr" />
          <Slider label="RAV4 PHEV SE" value={phevInsurance} min={600} max={3000} step={50}
            display={v => `${fmt(v)}/yr`} onChange={setPhevInsurance}
            note="Est. ~15–25% more than Volvo for newer PHEV. Get a quote to confirm." />
          <Slider label="RAV4 Hybrid XSE ⭐" value={rav4hInsurance} min={600} max={3000} step={50}
            display={v => `${fmt(v)}/yr`} onChange={setRav4hInsurance}
            note="RAV4 Hybrid insures competitively. Est. ~10–20% more than Volvo." />
          <Slider label="Crosstrek Hybrid Premium" value={hybridInsurance} min={600} max={3000} step={50}
            display={v => `${fmt(v)}/yr`} onChange={setHybridInsurance}
            note="Subarus are among the cheapest to insure. Est. ~10–15% more than Volvo." />
          ...
          <Slider label="RAV4 Hybrid XSE" value={rav4hMaint} min={300} max={1500} step={50} display={v => `${fmt(v)}/yr`} onChange={setRav4hMaint} note="~$500–850/yr" />
        ...
        <div>
          ...
          <div style={{ background: "white", borderRadius: 12, padding: 14, border: "1px solid #e5e7eb", marginBottom: 12 }}>
            ...
            <Bar label="RAV4 Hybrid XSE ⭐" sub="no charging needed" value={c.total.r} max={maxBar} color={COLORS.rav4h} />
          ...
          <div style={{ background: "#eff6ff", borderRadius: 12, padding: 12, border: "1px solid #bfdbfe", marginBottom: 12 }}>
            ...
            <ul style={{ margin: 0, paddingLeft: 15, fontSize: 11.5, color: "#1e3a8a", lineHeight: 1.8 }}>
              <li><strong>Best value in the lineup</strong> per multiple automotive reviewers who drove the full range.</li>
              <li>12.9" touchscreen, AWD standard, heated+ventilated seats, moonroof, hands-free liftgate.</li>
              <li><strong>No charging needed</strong> — 41 mpg EPA AWD, zero infrastructure dependency, zero range anxiety.</li>
              <li>Nearly identical price to RAV4 PHEV SE but simpler ownership.</li>
              <li>Limited trim adds panoramic roof + JBL audio but reviewers say it's not worth the premium.</li>
            </ul>
          </div>
          ...
          <div style={{ background: "#f5f3ff", borderRadius: 12, padding: 12, border: "1px solid #e9d5ff" }}>
            ...
            <ul style={{ margin: 0, paddingLeft: 15, fontSize: 11.5, color: "#4c1d95", lineHeight: 1.8 }}>
              <li><strong>Opportunity cost at 8–10%</strong> adds $4–6k to each new car scenario over 2 years — and compounds harder over 4–5 years. This is the strongest argument for keeping the Volvo short-term.</li>
              <li><strong>PHEV vs. Hybrid XSE:</strong> Same price, PHEV saves more on fuel but adds charging overhead. Your commute always exceeds EV range so the electric advantage is partial.</li>
              <li><strong>Volvo at 80k miles:</strong> Slide maintenance to $2,500–3,500 to stress-test one surprise repair.</li>
              <li><strong>CA gas spike:</strong> UC Davis projects +$1.21/gal by Aug 2026. Both RAV4s benefit significantly.</li>
              <li><strong>No federal PHEV/EV credit in 2026.</strong> Check CA CVRP at cleanvehiclerebate.org.</li>
            </ul>
          </div>
        </div>
      </div>
      ...
      <div style={{ background: "#fff7ed", borderRadius: 12, padding: 12, border: "1px solid #fed7aa", marginTop: 14, fontSize: 11, color: "#7c2d12" }}>
        <strong>Model notes:</strong> Volvo 24 mpg · Crosstrek Hybrid 36 mpg (non-plug-in) · RAV4 Hybrid XSE 41 mpg AWD EPA · RAV4 PHEV: EV covers first {phevEvRange} mi from full overnight charge, gas covers overflow · PHEV EV efficiency ~3.4 mi/kWh · Opportunity cost = net outlay × ((1+r)^n − 1), applied to purchase outlays only · Depreciation: declining-balance · $15k trade-in applied at purchase · Cash deal.
      </div>
    </div>
  );
}